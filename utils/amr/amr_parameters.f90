module amr_parameters
  ...
  real(dp),dimension(1:MAXOUT)::aout=HUGE(1.1D0)      ! Output expansion factors
  ...
  logical ::use_dx_loc = .false.        ! use local resolution as smoothing length
  logical ::use_b_turb_loc = .false.    ! use local turbulence forcing parameter
