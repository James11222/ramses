...
subroutine star_formation(ilevel)
  ...
  use constants, only: Myr2sec, Gyr2sec, mH, pi, rhoc, twopi, kB, M_sun
  use random
  use mpi_mod
  implicit none
#ifndef WITHOUTMPI
  ...
  real(dp)::mstar,dstar,tstar,nISM,nCOM,phi_t,phi_x,theta,sigs,scrit
  real(dp)::T2,nH,T_poly,T_fixed,cs2,cs2_fixed,cs2_poly,trel,t_dyn,t_ff,uvar
  real(dp)::alpha0
  real(dp)::rho_norm,T_const,T_cut,d_break,sharpness,eps_sf,rho_cut,eps_sf_cut
  real(dp)::sigma2
  ...
  real(dp)::dx,dx_loc,scale,vol_loc,dx_min,vol_min,dx_smooth,b_turb_loc
  real(dp)::mdebris
        ...
        if(sf_model > 0)then
           do i=1,ngrid
              ...
              if (ok(i)) then
                 ...
                 if (use_b_turb_loc) then; b_turb_loc = uold(ind_cell(i),ibturb); else; b_turb_loc = b_turb; end if
                 ! Smoothing scale
                 if (use_dx_loc) then; dx_smooth = dx_loc; else; dx_smooth = dx_min / aexp; end if
                 ...
                 if(ok(i)) then
                    SELECT CASE (sf_model)
                       ...
                       ! const SFE with density cutoff
                       CASE (1)
                          rho_cut = 4.235972431743681d-21/scale_d ! 1st quartile density from fiducial mff sim
                          eps_sf_cut = 0.1371535430985907 ! median SFE from fiducial mff sim
                          if(d>rho_cut) then
                             sfr_ff(i) = eps_sf_cut
                          else
                             sfr_ff(i) = 0.0
                        ...
                        ! const SFE with SFE cutoff
                        CASE (2)
                          eps_sf_cut = 0.1371535430985907 ! median SFE from fiducial mff sim
                          alpha0    = (15.0*(sigma2+cs2))/(pi*factG*d*dx_smooth**2) ! Virial parameter
                          M2        = max(sigma2/cs2,smallr) ! Turbulent Mach number
                          sigs      = log(1.0+(b_turb_loc**2)*(M2))
                          scrit     = log(alpha0*(1.0+(2*(M2**2)/(1.0+M2))))
                          eps_sf    = (eps_star/2.0)*exp(3.0/8.0*sigs)*(2.0-erfc((sigs-scrit)/sqrt(2.0*sigs)))
                          if(eps_sf>eps_sf_cut) then
                             sfr_ff(i) = eps_sf_cut
                          else
                             sfr_ff(i) = 0.0
                             ok(i)     = .false.
                        ...
                        ! don't allow star formation below eps_sf cutoff
                        CASE (4)
                          alpha0    = (15.0*(sigma2+cs2))/(pi*factG*d*dx_smooth**2) ! Virial parameter
                          M2        = max(sigma2/cs2,smallr) ! Turbulent Mach number
                          sigs      = log(1.0+(b_turb_loc**2)*(M2))
                        ...
                        CASE (5)
                          ...
                          T_const   = 8000.0
                          d_break   = 2.5d-22/scale_d
                          sharpness = 3.0
                          T_cut     = T_const*(1.0+(d/d_break)**sharpness)**(-1.0/sharpness)
