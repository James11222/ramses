     ...
     do j=1,np
        ...
        if(birth_time.lt.(current_time-t0))then
           ...
           if(sf_log_properties.and.(n_SN(j).gt.0.)) then
              ...
              enddo
              write(ilun,'(E24.12)',advance='no') uold(indp(j),1)
              do ivar=2,nvar
                 if(ivar.eq.ndim+2)then
                    e=0.0d0
                    do idim=1,ndim
                       e=e+0.5d0*uold(indp(j),idim+1)**2/max(uold(indp(j),1),smallr)
                    enddo
#if NENER>0
                    do irad=0,nener-1
                       e=e+uold(indp(j),inener+irad)
                    enddo
#endif
#ifdef SOLVERmhd
                    do idim=1,ndim
                       e=e+0.125d0*(uold(indp(j),idim+ndim+2)+uold(indp(j),idim+nvar))**2
                    enddo
#endif
                    ! Temperature
                    uvar=(gamma-1.0d0)*(uold(indp(j),ndim+2)-e)*scale_T2
                 else
                    uvar=uold(indp(j),ivar)
                 endif
                 write(ilun,'(E24.12)',advance='no') uvar/uold(indp(j),1)
              enddo
