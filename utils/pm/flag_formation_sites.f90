...
subroutine flag_formation_sites
  ...
  logical::ok
  logical::super_verbose
  logical::new_condition
  real(dp)::dx,dx_min,dist2,scale
  real(dp),dimension(1:npeaks)::peakd
  integer,dimension(1:npeaks)::ind_sort
  logical,dimension(1:ndim)::period
 
  ! This flag should only be turned on when examining smbh formation in depth
  super_verbose=.false.
  ...
  do j=npeaks,1,-1
     ...
     if (smbh)then
      ...
      new_condition=(ind_halo(jj).EQ.jj+ipeak_start(myid))
      if (super_verbose) then
        write(*, "(A)") "Clump has to be a halo: ", merge('True ', 'False', new_condition)
      end if
      ok=ok.and.new_condition
      ! Halo must have no existing sink
      new_condition = occupied(jj)==0
      if (super_verbose) then
        write(*, "(A)") "Halo must have no existing sink: ", merge('True ', 'False', new_condition)
      end if
      ok=ok.and.new_condition
      ! Halo mass has to be larger than some threshold
      new_condition = halo_mass(jj)>mass_halo_AGN*M_sun/(scale_d*scale_l**3)
      if (super_verbose) then
        write(*, "(A)") "Halo mass has to be larger than some threshold: ", merge('True ', 'False', new_condition)
      end if
      ok=ok.and.new_condition
      ! 4-cell ball stellar mass has to be larger than some threshold
      if(star.and.mass_star_AGN>0)then
          new_condition=clump_star4(jj)>mass_star_AGN*M_sun/(scale_d*scale_l**3)
          if (super_verbose) then
            write(*, "(A)") "4-cell ball stellar mass has to be larger than some threshold: ", merge('True ', 'False', new_condition)
          end if
          ok=ok.and.new_condition
      endif
      ! 4-cell ball mass has to be larger than some threshold
      new_condition=clump_mass4(jj)>mass_clump_AGN*M_sun/(scale_d*scale_l**3)
      if (super_verbose) then
        write(*, "(A)") "4-cell ball mass has to be larger than some threshold: ", merge('True ', 'False', new_condition)
      end if
      ok=ok.and.new_condition
      ! 4-cell ball av. density has to be larger than SF threshold
      new_condition=clump_mass4(jj)/(4d0/3d0*pi*(ir_cloud*dx_min/aexp)**3)>n_star/scale_nH
      if (super_verbose) then
        write(*, "(A)") "4-cell ball av. density has to be larger than SF threshold: ", merge('True ', 'False', new_condition)
      end if
      ok=ok.and.new_condition
      ! Peak density has to be larger than 10x star formation thresold
      new_condition=max_dens(jj)>10.0d0*n_star/scale_nH
      if (super_verbose) then
        write(*, "(A)") "Peak density has to be larger than 10x star formation thresold: ", merge('True ', 'False', new_condition)
      end if 
      ok=ok.and.new_condition
