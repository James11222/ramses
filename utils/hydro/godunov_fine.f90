...
subroutine add_viscosity_source_terms(ilevel)
  ...
  real(dp),dimension(1:nvector,1:ndim,1:ndim),save::velg,veld
  real(dp),dimension(1:nvector,1:ndim),save::dx_g,dx_d
  real(dp),dimension(1:nvector),save::divu_loc,phi_diss,curlnormsqu_loc,b_turb_loc
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        ! Compute curlnormsqu_loc (square of the norm of the curl)
        curlnormsqu_loc(1:ngrid)=0.0d0
        do i=1,ngrid
           curlnormsqu_loc(i) = (gradu_loc(i,2,3) - gradu_loc(i,3,2))**2 + (gradu_loc(i,3,1) - gradu_loc(i,1,3))**2 + (gradu_loc(i,1,2) - gradu_loc(i,2,1))**2
        enddo

        ! Compute b_turb (turbulence forcing)
        ! Based on eq. 23 of Federrath+2010
        b_turb_loc(1:ngrid)=1.0d0
        do i=1,ngrid
           b_turb_loc(i) = 1.0/3.0 + 2.0/3.0 * (divu_loc(i)**2 / (divu_loc(i)**2 + curlnormsqu_loc(i)))**3
        enddo
        ...
        do i=1,ngrid
           ...
           unew(ind_cell(i),ibturb) = uold(ind_cell(i),1)*b_turb_loc(i)
